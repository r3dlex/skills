---
name: setup-skills
description: Sets up an `## Agent skills` block in AGENTS.md/CLAUDE.md and `docs/agents/` so the engineering skills know this repo's issue tracker (GitHub, Azure DevOps, GitLab, or local markdown), triage label vocabulary, and domain doc layout. Run before first use of `to-issues`, `to-prd`, `triage`, `diagnose`, `tdd`, `improve-codebase-architecture`, or `zoom-out` — or if those skills appear to be missing context about the issue tracker, triage labels, or domain docs.
...
- **Issue tracker** — where issues live (GitHub by default; Azure DevOps, GitLab, and local markdown are also supported out of the box)
...
**Section A — Issue tracker.** If a `git remote` points at GitHub, default to GitHub. If it points at `dev.azure.com` or `visualstudio.com`, default to Azure DevOps Boards. If GitLab, default to GitLab. Otherwise, default to local markdown (`.scratch/`). Only ask the user if none of these are clearly right.
...
Auto-detect the file to edit (CLAUDE.md if it exists, else AGENTS.md), then write the `## Agent skills` block and the `docs/agents/` files. Use the seed templates in this skill folder as starting points, including `issue-tracker-ado.md` for Azure Boards. Mention which skills will now read from these files.
