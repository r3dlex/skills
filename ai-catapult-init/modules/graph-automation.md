...
3. **Run.** Dispatches via per-engine case block from `REPO_ROOT` with stdout/stderr appended to `${ENGINE}-out/refresh.log`: `graphify` → interpreter-detected `python -c "_rebuild_code(Path('.'))"` (no `--update` flag; graphify v0.4.x removed it); `graphwiki` → `graphwiki build . --update`; generic fallback → `"$ENGINE" . --update`. A `GRAPH_REFRESH_ENGINE_CMD` env var bypasses all engine logic for tests.
