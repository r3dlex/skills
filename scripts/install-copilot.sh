...
per_skill() { local out="$target/.github/copilot-instructions" description; mkdir -p "$out"; while IFS=$'\t' read -r name source; do [[ -n "$name" ]] || continue; description=$(grep -A1 '^description:' "$SKILLS_DIR/$source/SKILL.md" 2>/dev/null | tail -1 | sed 's/^ *//' || echo "Skill: $name"); { echo "# $name"; echo ''; echo "$description"; echo ''; flattened_skill_body "$SKILLS_DIR/$source/SKILL.md"; } > "$out/$name.md"; done <<< "$rows"; }
