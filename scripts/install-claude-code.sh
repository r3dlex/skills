...
install_to() { local dest="$1" rows; mkdir -p "$dest"; rows="$(catalog_rows claude-code --projection "$dest/catalog.json")" || return $?; while IFS=$'\t' read -r name source; do [[ -n "$name" ]] || continue; rm -rf "$dest/$name"; cp -R "$SKILLS_DIR/$source" "$dest/$name"; echo "  ✓ $name"; done <<< "$rows"; }
