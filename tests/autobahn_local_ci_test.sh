...
# Malformed step structures must block before any command executes.
for shape in empty-scalar empty-job misplaced-step colon-scalar zero-indent broken-quote; do
  r="$(repo)"
  printf '#!/bin/sh\ntouch ran\n' > "$r/tests/ok.sh"
  case "$shape" in
    empty-scalar) printf 'jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: |\n      - run: bash tests/ok.sh\n' ;;
    empty-job) printf 'jobs:\n  empty:\n    runs-on: ubuntu-latest\n    steps:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: bash tests/ok.sh\n' ;;
    misplaced-step) printf 'jobs:\n  t:\n    steps:\n      - run: bash tests/ok.sh\n    runs-on: ubuntu-latest\n      - run: bash tests/ok.sh\n' ;;
    colon-scalar) printf 'jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: bash tests/ok.sh key: value\n' ;;
    broken-quote) printf 'jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: "bash tests/ok.sh arg" "stuff"\n' ;;
    zero-indent) printf 'jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: |0\n          bash tests/ok.sh\n' ;;
  esac > "$r/.github/workflows/ci.yml"
  if bash "$LOCAL_CI" --root "$r" >/dev/null 2>&1 || [[ -e "$r/ran" ]]; then
    bad "$shape blocks before execution"
  else
    ok "$shape blocks before execution"
  fi
  rm -rf "$r"
done

# Workflow shell syntax must not become literal argv with different behavior.
for suffix in '# inline shell comment' '*' '~' '{one,two}' '(foo)'; do
  r="$(repo)"
  printf '#!/bin/sh\ntouch ran\n[ "$#" -gt 0 ]\n' > "$r/tests/check.sh"
  printf 'jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: |\n          bash tests/check.sh %s\n' "$suffix" > "$r/.github/workflows/ci.yml"
  if bash "$LOCAL_CI" --root "$r" >/dev/null 2>&1 || [[ -e "$r/ran" ]]; then
    bad "unmodeled shell syntax $suffix blocks before execution"
  else
    ok "unmodeled shell syntax $suffix blocks before execution"
  fi
  rm -rf "$r"
done
...
jobs:
  t:
    runs-on: ubuntu-latest
    steps:
      - run: 'bash tests/check.sh "a''b"'
YAML
if bash "$LOCAL_CI" --root "$r" >/dev/null 2>&1 || [[ -e "$r/ran" ]]; then
  bad "YAML single-quote escape blocks before execution"
else
  ok "YAML single-quote escape blocks before execution"
fi
rm -rf "$r"

for shape in indent7 indent8 body-dedent missing-space extra-space malformed-event duplicate-job; do
  r="$(repo)"
  printf '#!/bin/sh\ntouch ran\n' > "$r/tests/ok.sh"
  python3 - "$r/.github/workflows/ci.yml" "$shape" <<'PYTEST'
from pathlib import Path
import sys
text = 'jobs:\n  t:\n    runs-on: ubuntu-latest\n    steps:\n      - run: |\n          bash tests/ok.sh\n'
shape = sys.argv[2]
if shape.startswith('indent'): text = text.replace('          bash', ' ' * int(shape[-1]) + 'bash')
if shape == 'body-dedent': text += '         bash tests/ok.sh\n'
if shape == 'missing-space': text = text.replace('- run:', '-run:')
if shape == 'extra-space': text = text.replace('- run:', '-   run:')
if shape == 'malformed-event': text = 'on: [push\n' + text
if shape == 'duplicate-job': text += text.replace('jobs:\n', '').replace('  t:', '  t :')
