...
Read when choosing where BRDs, PRDs, tickets, and comments live. The adapter is selected at scaffold time and referenced by `ai-sdlc-init`; full operation semantics live in `setup-skills`. v3 adds an optional Jira adapter and explicit confirmation boundaries for any host adapter that mutates externally visible state.
